...
parser.add_argument("--n_seed", type=int, help="Number of seeds per game to use")
parser.add_argument("--n_ckpt", type=int, help="Number of checkpoints per game to use")
...
for game in games:
    for seed in range(1,n_seed+1):
        ...
        with ThreadPoolExecutor() as executor:
            params = [(f"{directory}/{game}", seed, ckpt) for ckpt in range(n_ckpt+1)]
...
file_path = f"./images_dataset/dataset_Full_{n_ckpt}_{n_seed}.pt"
